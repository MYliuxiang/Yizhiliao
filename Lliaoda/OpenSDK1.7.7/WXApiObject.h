...
/** 由用户微信号和AppID组成的唯一标识，发送请求时第三方程序必须填写，用于校验微信用户是否换号登入*/
...
/*! @brief 第三方向微信终端发起支付的訊息结构体
 *
 *  第三方向微信终端发起支付的訊息结构体，微信终端处理后会向第三方返回处理结果
 * @see PayResp
 */
...
/*! @brief 第三方程序向微信终端请求认证的訊息结构
 *
 * 第三方程序要向微信申请认证，并请求某些权限，需要调用WXApi的sendReq成员函数，
 * 向微信终端发送一个SendAuthReq訊息结构。微信终端处理完后会向第三方程序发送一个处理结果。
 * @see SendAuthResp
 */
@interface SendAuthReq : BaseReq
/** 第三方程序要向微信申请认证，并请求某些权限，需要调用WXApi的sendReq成员函数，向微信终端发送一个SendAuthReq訊息结构。微信终端处理完后会向第三方程序发送一个处理结果。
 * @see SendAuthResp
 ...
 *
 * 第三方程序要向微信申请认证，并请求某些权限，需要调用WXApi的sendReq成员函数，向微信终端发送一个SendAuthReq訊息结构。
...
/*! @brief 第三方程序发送訊息至微信终端程序的訊息结构体
 *
 * 第三方程序向微信发送信息需要传入SendMessageToWXReq结构体，信息类型包括文本訊息和多媒体訊息，
...
/** 发送訊息的类型，包括文本訊息和多媒体訊息两种，两者只能选择其一，不能同时发送文本和多媒体訊息 */
...
/*! @brief 微信终端向第三方程序请求提供内容的訊息结构体。
 *
 * 微信终端向第三方程序请求提供内容，微信终端会向第三方程序发送GetMessageFromWXReq訊息结构体，
 * 需要第三方程序调用sendResp返回一个GetMessageFromWXResp訊息结构体。
 */
...
/*! @brief 微信终端向第三方程序请求提供内容，第三方程序向微信终端返回的訊息结构体。
 *
 * 微信终端向第三方程序请求提供内容，第三方程序调用sendResp向微信终端返回一个GetMessageFromWXResp訊息结构体。
 */
 ...
 */
@property (nonatomic, retain) WXMediaMessage* message;
/** 向微信终端提供内容的訊息类型，包括文本訊息和多媒体訊息两种，两者只能选择其一，不能同时发送文本和多媒体訊息 */
@property (nonatomic, assign) BOOL bText;
@end
...
/*! @brief 微信通知第三方程序，要求第三方程序显示的訊息结构体。
 *
 * 微信需要通知第三方程序显示或处理某些内容时，会向第三方程序发送ShowMessageFromWXReq訊息结构体。
...
/*! @brief 微信通知第三方程序，要求第三方程序显示或处理某些訊息，第三方程序处理完后向微信终端发送的处理结果。
 *
 * 微信需要通知第三方程序显示或处理某些内容时，会向第三方程序发送ShowMessageFromWXReq訊息结构体。
 * 第三方程序处理完内容后调用sendResp向微信终端发送ShowMessageFromWXResp。
 */
...
/*! @brief 多媒体訊息中包含的图片数据对象
 *
 * 微信终端和第三方程序之间传递訊息中包含的图片数据对象。
...
/*! @brief 多媒体訊息中包含的音乐数据对象
 *
 * 微信终端和第三方程序之间传递訊息中包含的音乐数据对象。
...
/*! @brief 多媒体訊息中包含的視訊数据对象
 *
 * 微信终端和第三方程序之间传递訊息中包含的視訊数据对象。
...
/*! @brief 多媒体訊息中包含的网页数据对象
 *
 * 微信终端和第三方程序之间传递訊息中包含的网页数据对象。
 * @see WXMediaMessage
 */
...
/*! @brief 多媒体訊息中包含的App扩展数据对象
 *
 * 第三方程序向微信终端发送包含WXAppExtendObject的多媒体訊息，
 * 微信需要处理该訊息时，会调用该第三方程序来处理多媒体訊息内容。
...
/*! @brief 多媒体訊息中包含的表情数据对象
 *
 * 微信终端和第三方程序之间传递訊息中包含的表情数据对象。
 * @see WXMediaMessage
 */
...
/*! @brief 多媒体訊息中包含的地理位置数据对象
 *
 * 微信终端和第三方程序之间传递訊息中包含的地理位置数据对象。
 * @see WXMediaMessage
 */
...
/*! @brief 多媒体訊息中包含的文本数据对象
 *
 * 微信终端和第三方程序之间传递訊息中包含的文本数据对象。
 * @see WXMediaMessage
 */
