...
- (IBAction)completeAC:(id)sender {
    ...
    if (_textField.text.length != 10) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"您输入的手機號碼不正确！" delegate:nil cancelButtonTitle:@"確定" otherButtonTitles:nil, nil];
        [alert show];
        return;
    ...
    if (_ghuiLabel.text.length == 0) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"请填写公会号" delegate:nil cancelButtonTitle:@"確定" otherButtonTitles:nil, nil];
        [alert show];
        return;
    }
