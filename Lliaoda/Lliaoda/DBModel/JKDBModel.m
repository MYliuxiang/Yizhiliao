...
- (BOOL)save
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        NSLog(res?LXSring(@"插入成功"):LXSring(@"插入失败"));
    }];
    return res;
}
...
+ (BOOL)saveObjects:(NSArray *)array
{
    ...
    [jkDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (JKDBModel *model in array) {
            ...
            NSLog(flag?LXSring(@"插入成功"):LXSring(@"插入失败"));
            if (!flag) {
                res = NO;
                *rollback = YES;
                return;
...
- (BOOL)update
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        NSLog(res?LXSring(@"更新成功"):LXSring(@"更新失败"));
    }];
    return res;
}
...
+ (BOOL)updateObjects:(NSArray *)array
{
    ...
    [jkDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (JKDBModel *model in array) {
            ...
            NSLog(flag?LXSring(@"更新成功"):LXSring(@"更新失败"));
            if (!flag) {
                res = NO;
                *rollback = YES;
                return;
...
- (BOOL)deleteObject
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        res = [db executeUpdate:sql withArgumentsInArray:@[primaryValue]];
         NSLog(res?LXSring(@"删除成功"):LXSring(@"删除失败"));
    }];
    return res;
}
    ...
    [jkDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (JKDBModel *model in array) {
            ...
            BOOL flag = [db executeUpdate:sql withArgumentsInArray:@[primaryValue]];
             NSLog(flag?LXSring(@"删除成功"):LXSring(@"删除失败"));
            if (!flag) {
                res = NO;
                *rollback = YES;
                return;
...
+ (BOOL)clearTable
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        res = [db executeUpdate:sql];
        NSLog(res?LXSring(@"清空成功"):LXSring(@"清空失败"));
    }];
    return res;
}
