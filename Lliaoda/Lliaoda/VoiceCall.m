...
- (void)onInviteAcceptedByPeer
{
    [self stop];
    NSDictionary *params;
    params = @{@"channel":self.channel,@"type":@1};
...
- (IBAction)closeVideo:(id)sender {
    ...
    if(self.iscalling){
        ...
        LGAlertView *lg = [[LGAlertView alloc] initWithTitle:LXSring(@"確定退出") message:@"確定退出語音通話吗？" style:LGAlertViewStyleAlert buttonTitles:nil cancelButtonTitle:LXSring(@"取消") destructiveButtonTitle:LXSring(@"確定") delegate:nil];
