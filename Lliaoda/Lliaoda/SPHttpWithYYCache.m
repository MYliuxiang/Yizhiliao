...
{
    [[self alloc] requestWithUrl:urlStr withDic:nil requestType:RequestTypeGet isCache:NO cacheKey:nil imageKey:nil withData:nil upLoadProgress:nil success:^(id requestDic) {
        success(requestDic);
...
+(void)getRequestCacheUrlStr:(NSString *)urlStr success:(SuccessBlock)success failure:(FailureBlock)failuer{
    ...
    [[self alloc] requestWithUrl:urlStr withDic:nil requestType:RequestTypeGet isCache:YES cacheKey:urlStr imageKey:nil withData:nil upLoadProgress:nil success:^(id requestDic) {
        success(requestDic);
    } failure:^(NSError *error) {
...
{
    [[self alloc] requestWithUrl:urlStr withDic:parameters requestType:RequestTypePost isCache:NO cacheKey:urlStr imageKey:nil withData:nil upLoadProgress:nil success:^(id requestDic) {
        success(requestDic);
    } failure:^(NSError *error) {
...
{
    [[self alloc] requestWithUrl:urlStr withDic:parameters requestType:RequestTypePost isCache:YES cacheKey:urlStr imageKey:nil withData:nil upLoadProgress:nil success:^(id requestDic) {
        success(requestDic);
    } failure:^(NSError *error) {
...
//            failure(LXSring(@"没有网络"));
    ...
    if (requestType == RequestTypeGet) {
        [session GET:url parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            [self dealWithResponseObject:responseObject cacheUrl:cacheUrl cacheData:cacheData isCache:isCache cache:cache cacheKey:cacheKey success:^(id requestDic) {
    ...
    if (requestType == RequestTypeUpLoad) {
        [session POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
            ...
            [self dealWithResponseObject:responseObject cacheUrl:cacheUrl cacheData:cacheData isCache:isCache cache:nil cacheKey:nil success:^(id requestDic) {
                success(requestDic);
            } failure:^(NSError *error) {
                   failure(error);
            }];
