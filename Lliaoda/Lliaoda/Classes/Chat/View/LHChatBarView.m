...
//   self.textView.frame = CGRectMake(10, 7.5, SCREEN_W - 20, self.height - 15);
...
//    [self haveVideoLoadData];
    ...
    self.moreBtn.frame = CGRectMake(10, self.height - kChatBatItemWH - 11, kChatBatItemWH, kChatBatItemWH);
...
//                _videoEnabled = [result[@"data"][@"config"][@"videoEnabled"] boolValue];
...
- (void)moreViewTakePicAction:(LHChatBarMoreView *)moreView {
    if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:LXSring(@"错误")
                                                            message:LXSring(@"设备没有摄像头")
                                                           delegate:nil
                                                  cancelButtonTitle:LXSring(@"好的")
...
- (void)textViewDidChange:(UITextView *)textView {
    ...
    if (size.height >= maxHeight) {
        ...
        textView.scrollEnabled = NO;    // 不允许滚动，当textview的大小足以容纳它的text的时候，需要設定scrollEnabed为NO，否则会出现光标乱滚动的情况
    }
