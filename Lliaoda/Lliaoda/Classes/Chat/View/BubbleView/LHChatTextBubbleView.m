...
- (void)setMessageModel:(Message *)messageModel {
    ...
    // 去除超链接
//     _urlMatches = [_detector matchesInString:messageModel.content options:0 range:NSMakeRange(0, messageModel.content.length)];
//     [self highlightLinksWithIndex:NSNotFound];
}

#pragma mark - 私有
+ (NSAttributedString *)processModel:(Message *)model {
...
-(void)bubbleViewPressed:(id)sender {
    ...
    [self highlightLinksWithIndex:NSNotFound];
    [self routerEventWithName:kRouterEventTapEventName userInfo:@{kMessageKey : self.messageModel}];
...
//        [self routerEventWithName:kRouterEventBubbleLongTapEventName
...
//
        ...
        UIMenuItem *item1 = [[UIMenuItem alloc]initWithTitle:@"复制" action:@selector(myCut:)];
...
-(BOOL)canPerformAction:(SEL)action withSender:(id)sender{
    ...
    if (action == @selector(myCut:)||action == @selector(myDelete:)||action == @selector(myMore:)) {
        return YES;
    }
    return NO;
}
...
- (void)myCut:(UIMenuController *) menu
{
    ...
    NSString *str = self.messageModel.content;
    paste.string = str;
