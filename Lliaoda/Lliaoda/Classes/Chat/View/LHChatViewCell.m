...
- (void)setMessageModel:(Message *)model {
...
- (void)retryButtonPressed:(UIButton *)sender {
    [self routerEventWithName:kRouterEventChatResendEventName
                     userInfo:@{kShouldResendCell : self.messageModel}];
    
}

#pragma mark - private
- (void)setupSubviewsForMessageModel:(Message *)messageModel
{
    ...
    if (messageModel.isSender) {
        ...
        [_retryButton setImage:[UIImage imageNamed:@"shibai"] forState:UIControlStateNormal];
...
- (LHChatBaseBubbleView *)bubbleViewForMessageModel:(Message *)messageModel {
...
+ (CGFloat)bubbleViewHeightForMessageModel:(Message *)messageModel {
    switch (messageModel.type) {
        case MessageBodyType_Text: {
