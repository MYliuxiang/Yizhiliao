...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (!cell.longPressGestureBlock) {
        ...
        cell.longPressGestureBlock = ^(UIImage *image) {
            [UIActionSheet showInView:self.view withTitle:LXSring(@"保存图片到相薄") cancelButtonTitle:LXSring(@"取消") destructiveButtonTitle:nil otherButtonTitles:@[LXSring(@"保存")] tapBlock:^(UIActionSheet * _Nonnull actionSheet, NSInteger buttonIndex) {
                if (!buttonIndex) {
                    dispatch_async(dispatch_get_global_queue(0, 0), ^{
                        UIImageWriteToSavedPhotosAlbum(image, nil, nil,nil);
                        dispatch_sync(dispatch_get_main_queue(), ^{
                            [weakSelf showReminderWithContent:LXSring(@"图片已保存到相薄")];
                        });
                    });
