...
- (IBAction)sendAc:(id)sender {
    ...
    if ([_onetextField.text integerValue] < 100) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:DTLocalizedString(@"提示", nil) message:@"亲！红包金额不能低于100鑽石" delegate:nil cancelButtonTitle:DTLocalizedString(@"確定", nil) otherButtonTitles:nil, nil];
        [alert show];
        return;
    }
    if ([_twotextField.text integerValue] < 1) {
        
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:DTLocalizedString(@"提示", nil) message:@"亲！红包个数不能小于1" delegate:nil cancelButtonTitle:DTLocalizedString(@"確定", nil) otherButtonTitles:nil, nil];
        [alert show];
        return;
    }
        ...
        if(result){
            if ([[result objectForKey:@"result"] integerValue] == 0) {
                ...
                if ([[result objectForKey:@"result"] integerValue] == 8) {
                    ...
                    LGAlertView *lg = [[LGAlertView alloc] initWithTitle:DTLocalizedString(@"购买鑽石", nil) message:@"老闆，鑽石不夠啦～儲值後立刻打賞！" style:LGAlertViewStyleAlert buttonTitles:nil cancelButtonTitle:@"［暫不用］" destructiveButtonTitle:@"［去儲值］" delegate:nil];
